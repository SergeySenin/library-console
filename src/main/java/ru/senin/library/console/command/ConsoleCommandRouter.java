...
/**
 * Маршрутизатор верхнеуровневых команд консольного приложения.
 *
 * <p>Сопоставляет уже валидную строковую команду с зарегистрированным действием и
 * возвращает {@link CommandRoutingResult}.
 * Реестр команд собирается один раз при создании объекта, после этого работает как неизменяемый механизм маршрутизации.
 *
 * <p>Класс не читает ввод, не валидирует ввод и не реализует предметные книжные сценарии.
 * Он отделён и от runner-а, и от handler-слоя, чтобы маршрутизация оставалась самостоятельной ответственностью.
 */
